...
#define imx214_SENSOR_NAME "imx214_hct"
#define PLATFORM_DRIVER_NAME "msm_camera_imx214_htc"

#define DUAL_CAL_OTP_SIZE 1024
int32_t imx214_read_otp_memory(uint8_t *otpPtr, struct msm_sensor_ctrl_t *s_ctrl);
...
static const struct of_device_id imx214_dt_match[] = {
	{.compatible = "htc,imx214_htc", .data = &imx214_s_ctrl},
	{}
};
...
static const char *imx214NAME = "imx214_htc";
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if (first)
    {
        ...
        imx214_read_otp_memory(otp_mem, s_ctrl);
        if (rc<0) {
...
static int imx214_read_fuseid32(struct sensorb_cfg_data32 *cdata,
	struct msm_sensor_ctrl_t *s_ctrl)
{
    int32_t rc = 0;
    int32_t i = 0, j = 0;
...
int32_t imx214_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(rc == 0)
	{
	    if(first == 0)
	    {
			...
			#ifdef CONFIG_COMPAT
			rc1 = imx214_read_fuseid32(NULL, s_ctrl);
			#else
...
static struct msm_sensor_fn_t imx214_sensor_func_tbl = {
	...
	.sensor_i2c_read_fuseid32 = imx214_read_fuseid32,
};
