...
static int g_bid;
static int __init board_set_bid(char *get_hboot_bid)
{
        if(strlen(get_hboot_bid))
                g_bid = atoi(get_hboot_bid);
        else
                g_bid = 0;
        return 1;
}
__setup("pcbid.bid=", board_set_bid);

int board_get_bid(void)
{
	return g_bid;
}
EXPORT_SYMBOL(board_get_bid);
