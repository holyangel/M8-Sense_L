...
#include <../../../arm64/include/asm/debug_display.h>
...
static int lm3697_init_registers(struct lm3697_data *drvdata)
{
	...
	platform_write_i2c_block(drvdata->adapter, drvdata->addr, LM3697_BRIGHTNESS_CFG_REG, 0x01, &drvdata->bri_cfg);
...
void lm3697_set_brightness(struct lm3697_data *drvdata, int brt_val)
{
	...
	if (drvdata->bri_cfg)
		code = brt_val * 2047 / MAX_BRIGHTNESS;
	else {
		if (!drvdata->brt_code_table) {
			pr_err("%s:%d, code table is empty\n",__func__, __LINE__);
			return;
		}
		code = drvdata->brt_code_table[brt_val];
	}
	brt_LSB = code % 0x7;
	brt_MSB = (code >> 3) & 0xFF;
...
static int lm3697_get_dt_data(struct device *dev, struct lm3697_data *drvdata)
{
	...
	rc = of_property_read_u32(of_node, "bri-cfg", &tmp);
	if (rc) {
		pr_err("%s:%d, dt not specified\n",
						__func__, __LINE__);
	}
	drvdata->bri_cfg = (!rc ? tmp : 1); 
	pr_debug("%s : bri_cfg=0x%x\n",__func__, drvdata->bri_cfg);
