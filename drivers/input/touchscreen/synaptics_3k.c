...
static void syn_handle_block_touch(struct synaptics_ts_data *ts, int enable)
{
       ...
       if (ts->hall_block_touch_event) {
               ret = cancel_delayed_work(&syn_block_touch_work);
               syn_block_touch(ts, 0);
       }
       if (enable) {
...
static void syn_set_cover_func(struct work_struct *work)
{
	...
	if (ts->cover_enable) {
		...
		{
			if (ts->packrat_number >= SYNAPTICS_FW179_3508_COVER) {
				ts->f12_ctrl10[1] = ts->cover_setting[3];
				ts->f12_ctrl10[0] = ts->cover_setting[2];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_10_offset, ts->f12_ctrl10, 2);
				if (ret < 0)
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:1", __func__);
				ts->f12_ctrl15[1] = ts->cover_setting[1];
				ts->f12_ctrl15[0] = ts->cover_setting[0];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_15_offset, ts->f12_ctrl15, 2);
				if (ret < 0)
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
				ts->f01_ctrl03[0] = ts->cover_setting[4];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x01, CONTROL_BASE) + 3, ts->f01_ctrl03, 1);
				if (ret < 0)
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:3", __func__);
			} else if (ts->packrat_number >= SYNAPTICS_FW_3508_COVER) {
		...
		{
			if (ts->packrat_number >= SYNAPTICS_FW179_3508_COVER) {
				ts->f12_ctrl10[1] = ts->uncover_setting[3];
				ts->f12_ctrl10[0] = ts->uncover_setting[2];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_10_offset, ts->f12_ctrl10, 2);
				if (ret < 0)
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:1", __func__);
				ts->f12_ctrl15[1] = ts->uncover_setting[1];
				ts->f12_ctrl15[0] = ts->uncover_setting[0];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x12, CONTROL_BASE) + ts->ctrl_15_offset, ts->f12_ctrl15, 2);
				if (ret < 0)
					i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
				ts->f01_ctrl03[0] = ts->uncover_setting[4];
				ret = i2c_syn_write(ts->client, get_address_base(ts, 0x01, CONTROL_BASE) + 3, ts->f01_ctrl03, 1);
				if (ret < 0)
				i2c_syn_reset_handler(ts, ts->i2c_err_handler_en, "w:3", __func__);
			} else if (ts->packrat_number >= SYNAPTICS_FW_3508_COVER) {
...
static int synaptics_input_register(struct synaptics_ts_data *ts)
{
	...
	if (ts->htc_event == SYN_AND_REPORT_TYPE_B) {
		input_mt_init_slots(ts->input_dev, ts->finger_support, 0);
	} else {
...
static void synaptics_ts_report_func(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0)
		...
		if (ret >= 0)
			...
			memset(&ts->report_data[0], 0x0, 4 * ts->x_channel * ts->y_channel);
...
static int synaptics_parse_config(struct synaptics_ts_data *ts, struct synaptics_i2c_rmi_platform_data *pdata)
{
	...
	while ((pp = of_get_next_child(node, pp))) {
		...
		{
			if (ts->packrat_number >= SYNAPTICS_FW179_3508_COVER) {
				if (of_property_read_u32_array(pp, "cover_setting", cover, 5) == 0) {
					cfg_table[i].cover_setting[0] = cover[0];
					cfg_table[i].cover_setting[1] = cover[1];
					cfg_table[i].cover_setting[2] = cover[2];
					cfg_table[i].cover_setting[3] = cover[3];
	...
	if (i <= cnt) {
		...
		{
			if (ts->packrat_number >= SYNAPTICS_FW179_3508_COVER)
				memcpy(pdata->cover_setting, cfg_table[i].cover_setting, 5);
			else
				memcpy(pdata->cover_setting, cfg_table[i].cover_setting, 2);
		}
...
static int synaptics_ts_suspend(struct device *dev)
{
	...
	if (ts->power)
		...
		if (ts->packrat_number >= SYNAPTICS_FW_NOCAL_PACKRAT) {
#if defined(CONFIG_SYNC_TOUCH_STATUS)
			ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x01, CONTROL_BASE), 0x02); 
			pr_info("[TP] %s: Sleep without calibration 0x02\n", __func__);
			if (ret < 0)
				i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "sleep: 0x02", __func__);
#else
			ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x01, CONTROL_BASE), 0x01); 
			pr_info("[TP] %s: Sleep with calibration 0x01\n", __func__);
			if (ret < 0)
				i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "sleep: 0x01", __func__);
#endif
		} else {
...
static void synaptics_ts_init_async(void *unused, async_cookie_t cookie)
{
