...
struct sdhci_pinctrl_data {
	...
	struct pinctrl_state	*pins_active_ddr50_sdr104;
...
static int sdhci_msm_setup_hifreq_pinctrl(struct sdhci_msm_pltfm_data *pdata)
{
	...
	ret = pinctrl_select_state(pdata->pctrl_data->pctrl,
			pdata->pctrl_data->pins_active_ddr50_sdr104);

	pr_info("Set SD DDR50_SDR104 state return : %d\n", ret);
...
static int sdhci_msm_setup_hifreq_pins(struct sdhci_msm_pltfm_data *pdata)
{
	...
	if (pdata->pctrl_data && pdata->pctrl_data->pins_active_ddr50_sdr104)
		...
		struct sdhci_msm_pltfm_data *pdata)
{
	...
	pctrl_data->pins_active_ddr50_sdr104 = pinctrl_lookup_state(
			pctrl_data->pctrl, "active_ddr50_sdr104");
	if (IS_ERR(pctrl_data->pins_active_ddr50_sdr104)) {
		ret = PTR_ERR(pctrl_data->pins_active_ddr50_sdr104);
		dev_err(dev, "Could not get active pinstates for DDR50_SDR104, err:%d\n", ret);
		pctrl_data->pins_active_ddr50_sdr104 = NULL;
		
		ret = 0;
	} else
		dev_info(dev, "Get active pinstates for DDR50_SDR104\n");
...
static void sdhci_msm_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (sup_clock != msm_host->clk_rate) {
		if (is_sd_platform(msm_host->pdata) &&
			(curr_ios.timing >= MMC_TIMING_UHS_SDR12 &&
...
static void sdhci_msm_shutdown(struct platform_device *pdev)
{
	...
	} else if (is_sd_platform(msm_host->pdata)) {
