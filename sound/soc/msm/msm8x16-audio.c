...
static bool hs_gpio_config = false;
#else
static bool hs_gpio_config = true;
#endif
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0])
	{
		...
		pr_info("msm_rcv_amp_put: 1\n");
	}
	else
	{
		
		htc_amp_mask &= ~HTC_RCV_AMP;
		pr_info("msm_rcv_amp_put: 0\n");
	}
			     ...
			     struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		if (spkl_amp_on || spkr_amp_on) {
			pr_info("%s:mi2s amp on Factory use Left (up): %d, Right (bottom): %d \n",__func__, spkl_amp_on, spkr_amp_on);
			if (spkl_amp_on)
...
static const struct snd_soc_dapm_route htc_hs_amp_route[] = {
	...
	{"SPK_AMP_EN", "Switch", "HPHR PA"},
	{"SPK_AMP_EN", "Switch", "HPHL PA"},
...
static int msm_audrx_init_wcd(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_ignore_suspend(dapm, "RCV_AMP");
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: dev name %s, id:%d register card done\n", __func__,
			card->name, pdev->id);
